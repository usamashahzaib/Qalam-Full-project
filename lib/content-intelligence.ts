...
export async function analyzeContentWithAi(
  content: string,
  role: string,
  voiceProfile?: { sample_posts?: string[] }
): Promise<ContentScore> {
  const systemPrompt = `You are a LinkedIn content quality analyst. Score this post on 5 dimensions (0-100 each) and provide specific feedback.

SCORING CRITERIA:
- HOOK (0-100): Does the first line stop the scroll? Is it specific, surprising, or data-driven? Generic = low score.
- SPECIFICITY (0-100): Are there real numbers, examples, or stories? Vague advice = low score.
- CTA (0-100): Does the ending invite real engagement? "Let me know your thoughts" = 40. Specific question = 80+.
- HUMAN-LIKENESS (0-100): Does it sound like a real person wrote it? AI-sounding = low score.
- VOICE FIT (0-100): Does it match the writer's established voice and expertise?
...
Return ONLY valid JSON in this exact format:
{
  ...
  const userPrompt = `POST TO ANALYZE:
${content}

WRITER ROLE: ${role}`

  try {
    const result = await callAi(systemPrompt, userPrompt, { json: true, timeout: 8000 })
    const parsed = JSON.parse(result)

    const overall = Math.round(
      (parsed.hook + parsed.specificity + parsed.cta + parsed.humanLikeness + parsed.voiceFit) / 5
    )
    ...
    return {
      ...
      cta: parsed.cta,
      humanLikeness: parsed.humanLikeness,
      voiceFit: parsed.voiceFit,
      feedback: parsed.feedback || [],
      aiSlopDetected: parsed.aiSlopDetected || false,
    }
  } catch {
...
function fallbackScoring(content: string): ContentScore {
  const hasNumbers = /\d+/.test(content)
  const hasQuestion = /\?/.test(content)
  const hasStory = /I |my |we |our /.test(content)

  return {
    overall: 60,
    hook: hasStory ? 65 : 50,
    specificity: hasNumbers ? 70 : 45,
    cta: hasQuestion ? 70 : 50,
    humanLikeness: hasStory ? 70 : 50,
    voiceFit: 60,
    feedback: ["AI quality analysis temporarily unavailable. Basic scoring applied."],
