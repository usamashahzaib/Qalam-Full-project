export const plans = [
  {
    name: 'Free',
    monthlyPrice: 0,
    annualPrice: undefined,
    draftsPerMonth: 5,
    carouselsPerMonth: 0,
    researchPerMonth: 0,
    annualLabel: 'Free forever',
    description: 'Try the real workflow. No card, no time limit - just a capped starter workspace.',
    features: ['Basic AI Writer', 'Hook Generator'],
    cta: 'Start free',
    badge: 'No card required',
    limits: { drafts: 5, workspaces: 1, carousels: 0, competitorRuns: 0, voiceProfiles: 0 }
  },
  {
    name: 'Solo',
    monthlyPrice: 499,
    annualPrice: 333,
    draftsPerMonth: 50,
    carouselsPerMonth: 3,
    researchPerMonth: 0,
    annualLabel: 'PKR 333/mo billed annually',
    description: 'For individual professionals building a consistent LinkedIn presence.',
    features: ['AI Writer', '3 Carousel Templates', 'Voice Profile', 'Full Analytics'],
    cta: 'Get Solo',
    badge: 'Most popular',
    limits: { drafts: 50, workspaces: 1, carousels: 3, competitorRuns: 0, voiceProfiles: 1 }
  },
  {
    name: 'Pro',
    monthlyPrice: 990,
    annualPrice: 660,
    draftsPerMonth: Infinity,
    carouselsPerMonth: 15,
    researchPerMonth: 5,
    annualLabel: 'PKR 660/mo billed annually',
    description: 'For serious creators and consultants who need full AI power and research.',
    features: ['AI Writer', '15 Carousel Templates', 'Voice Training', 'AI Strategist', 'Priority Queue', 'Advanced Analytics'],
    cta: 'Get Pro',
    badge: 'Best value',
    limits: { drafts: Infinity, workspaces: 1, carousels: 15, competitorRuns: 5, voiceProfiles: 5 }
  },
  {
    name: 'Agency Starter',
    monthlyPrice: 4990,
    annualPrice: 3330,
    draftsPerMonth: Infinity,
    carouselsPerMonth: 50,
    researchPerMonth: 25,
    workspaces: 3,
    annualLabel: 'PKR 3,330/mo billed annually',
    description: 'For teams managing multiple clients and brands.',
    features: ['Everything in Pro', '3 Workspaces', 'Approval Workflow', 'Team Analytics', 'White-label Export'],
    cta: 'Get Agency Starter',
    badge: 'For teams',
    limits: { drafts: Infinity, workspaces: 3, carousels: 50, competitorRuns: 25, voiceProfiles: 25 }
  },
  {
    name: 'Agency Growth',
    monthlyPrice: 9990,
    annualPrice: 6660,
    draftsPerMonth: Infinity,
    carouselsPerMonth: Infinity,
    researchPerMonth: Infinity,
    workspaces: Infinity,
    annualLabel: 'PKR 6,660/mo billed annually',
    description: 'For scaled agencies that need unlimited client operations.',
    features: ['Everything in Agency Starter', 'Unlimited Workspaces', 'Unlimited Carousels', 'Unlimited Research'],
    cta: 'Get Agency Growth',
    badge: 'Scale',
    limits: { drafts: Infinity, workspaces: Infinity, carousels: Infinity, competitorRuns: Infinity, voiceProfiles: Infinity }
  }
];

export const annualFraming = '4 months free';
export const annualSavingsPercent = 33;

// Helper to get plan by name
export function getPlanByName(name: string) {
  return plans.find(p => p.name === name) || plans[0];
}

// Helper to check if a feature is available on a plan
export function hasFeature(planName: string, feature: string): boolean {
  const plan = getPlanByName(planName);
  const agency = plan.name.startsWith('Agency');
  switch (feature) {
    case 'scheduling': return plan.name !== 'Free';
    case 'voiceProfile': return plan.name !== 'Free';
    case 'analytics': return plan.name !== 'Free';
    case 'carousel': return plan.limits.carousels > 0;
    case 'competitorResearch': return plan.name === 'Pro' || agency;
    case 'approvalWorkflow': return plan.name === 'Pro' || agency;
    case 'export': return plan.name === 'Pro' || agency;
    case 'whiteLabel': return agency;
    case 'teamSeats': return agency;
    default: return false;
  }
}

export interface PricingPlan {
  plan: string;
  monthlyPkr: number;
  annualPkrPerMonth?: number;
  period: string;
  description: string;
  features: string[];
  cta: string;
  href: string;
  highlighted?: boolean;
  badge?: string;
  featureStatus: 'live' | 'beta' | 'coming_soon';
}

export const PLANS: PricingPlan[] = plans.map(plan => ({
  plan: plan.name,
  monthlyPkr: plan.monthlyPrice,
  annualPkrPerMonth: plan.annualPrice,
  period: plan.monthlyPrice === 0 ? 'forever' : 'mo',
  description: plan.description,
  features: plan.features,
  cta: plan.cta,
  href: plan.name.startsWith('Agency') ? '/contact' : '/auth',
  highlighted: plan.badge === 'Most popular',
  badge: plan.badge,
  featureStatus: plan.name.startsWith('Agency') ? 'beta' : 'live',
}));

export const PLAN_PRICES: Record<string, { monthly: number; annual: number }> = Object.fromEntries(
  plans.map(plan => [plan.name, { monthly: plan.monthlyPrice, annual: plan.annualPrice ?? 0 }])
);

export const PLAN_FEATURES: Record<string, string[]> = Object.fromEntries(
  plans.map(plan => [plan.name, plan.features])
);

export const COMPARISON_ROWS: {
  label: string;
  free: string;
  solo: string;
  pro: string;
  agencyStarter: string;
  agencyGrowth: string;
}[] = [
  { label: 'AI drafts', free: '5/month', solo: '50/month', pro: 'Unlimited', agencyStarter: 'Unlimited', agencyGrowth: 'Unlimited' },
  { label: 'Post scheduling', free: '-', solo: 'Live', pro: 'Live', agencyStarter: 'Live', agencyGrowth: '-' },
  { label: 'Voice profiles', free: '-', solo: '1', pro: '5', agencyStarter: '25', agencyGrowth: 'Unlimited' },
  { label: 'Scoring', free: 'Basic', solo: 'Basic', pro: 'Full', agencyStarter: 'Full', agencyGrowth: '-' },
  { label: 'Carousel generation', free: '-', solo: '3/month', pro: '15/month', agencyStarter: '50/month', agencyGrowth: 'Unlimited' },
  { label: 'Competitor research', free: '-', solo: '-', pro: '5 runs/month', agencyStarter: '25 runs/month', agencyGrowth: 'Unlimited' },
  { label: 'Approval workflow', free: '-', solo: '-', pro: 'Live', agencyStarter: 'Live', agencyGrowth: '-' },
  { label: 'Analytics', free: '-', solo: 'Basic', pro: 'Full', agencyStarter: 'Full', agencyGrowth: '-' },
  { label: 'Export', free: '-', solo: '-', pro: 'PDF and text', agencyStarter: 'PDF and text', agencyGrowth: '-' },
  { label: 'Client workspaces', free: '-', solo: '-', pro: '-', agencyStarter: '3', agencyGrowth: 'Unlimited' },
  { label: 'Team seats', free: '1', solo: '1', pro: '1', agencyStarter: '5', agencyGrowth: 'Unlimited' },
  { label: 'Monthly price', free: 'Free', solo: 'PKR 499', pro: 'PKR 990', agencyStarter: 'PKR 4,990', agencyGrowth: 'PKR 9,990' },
];

export const formatPkr = (amount: number): string => {
  if (amount === 0) return 'Free';
  return `PKR ${amount.toLocaleString('en-PK')}`;
};
