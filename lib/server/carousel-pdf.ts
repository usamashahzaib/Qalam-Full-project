...
import type { CarouselTheme } from "@/lib/carousel-design"

export type CarouselPdfSlide = { title?: string; bullets?: string[]; designHint?: string }
...
function drawCoverSlide(
  ...
  const contentY = 380
  page.drawText("CAROUSEL", { x: 80, y: contentY + 290, size: 22, font: bold, color: accent, opacity: 0.85 })

  const titleLines = wrap(title, 30)
  titleLines.slice(0, 4).forEach((line, i) => {
    page.drawText(line, { x: 80, y: contentY + 220 - i * 74, size: 66, font: bold, color: primary, maxWidth: 900 })
  })
  ...
  page.drawText(`${slideNum} / ${total}`, { x: 900, y: 80, size: 20, font: bold, color: muted.color, opacity: muted.opacity })
  page.drawText("Qalam - byqalam.com", { x: 80, y: 80, size: 20, font: bold, color: muted.color, opacity: muted.opacity * 0.7 })
}
...
function drawContentSlide(
  ...
  page.drawText(numStr, { x: numStr.length === 1 ? 104 : 99, y: 990, size: 22, font: bold, color: badgeText })
  page.drawText(`of ${total} slides`, { x: 148, y: 991, size: 18, font: regular, color: muted.color, opacity: muted.opacity })
  ...
  titleLines.slice(0, 3).forEach((line, i) => {
    page.drawText(line, { x: 80, y: titleY - i * 56, size: 44, font: bold, color: primary, maxWidth: 920 })
  })
  ...
  bodyLines.slice(0, 6).forEach((line, i) => {
    page.drawText(line, { x: 80, y: bodyStartY - i * 46, size: 30, font: regular, color: secondary.color, opacity: secondary.opacity, maxWidth: 920 })
  })

  page.drawLine({ start: { x: 80, y: 170 }, end: { x: 1000, y: 170 }, thickness: 1, color: divider.color, opacity: divider.opacity })
  ...
  page.drawText("Qalam - byqalam.com", { x: 80, y: 80, size: 18, font: bold, color: muted.color, opacity: muted.opacity * 0.7 })
}
...
function drawCtaSlide(
  ...
  const centerY = 580
  page.drawText("KEY TAKEAWAY", { x: 540 - 120, y: centerY + 210, size: 21, font: bold, color: accent, opacity: 0.9 })

  const titleLines = wrap(title, 32)
  titleLines.slice(0, 3).forEach((line, i) => {
    const textWidth = line.length * 24
    page.drawText(line, { x: Math.max(80, 540 - textWidth / 2), y: centerY + 140 - i * 60, size: 46, font: bold, color: primary, maxWidth: 920 })
  })
  ...
  if (content) {
    ...
    bodyLines.slice(0, 3).forEach((line, i) => {
      ...
      page.drawText(line, {
        ...
        size: 30, font: regular, color: secondary.color, opacity: secondary.opacity, maxWidth: 920,
      })
    })
  }
  ...
  page.drawText("Follow for more", { x: 456, y: btnY, size: 28, font: bold, color: badgeText })

  page.drawText(`${slideNum} / ${total}`, { x: 900, y: 80, size: 20, font: bold, color: muted.color, opacity: muted.opacity })
  page.drawText("Qalam - byqalam.com", { x: 80, y: 80, size: 20, font: bold, color: muted.color, opacity: muted.opacity * 0.7 })
}

export async function buildCarouselPdf(slides: CarouselPdfSlide[], theme: CarouselTheme): Promise<Uint8Array> {
  ...
  slides.forEach((slide, index) => {
    ...
    if (index === 0) {
      drawCoverSlide(pdf, title, slideNum, total, theme, bold)
