...
const getEncryptionKey = () => createHash("sha256").update(env.oauthStateSecret).digest()
...
export const createSignedToken = (payload: object): string => {
  ...
  const signature = createHmac("sha256", env.oauthStateSecret).update(encoded).digest("base64url")
...
export const readSignedToken = <T>(token: string, invalidErrorCode: string): T => {
  ...
  const expectedSig = createHmac("sha256", env.oauthStateSecret).update(encoded).digest("base64url")
