-- Pending migration: auth column, RLS policies, and write RPCs
-- Safe to run multiple times (all statements are idempotent)
-- Run in Supabase SQL Editor: https://supabase.com/dashboard/project/rnyjlgxrvtzdavsfocmi/sql/new

-- ============================================================
-- Part 1: Auth column + user lookup function (from 0008)
-- ============================================================
...
-- Drop before recreate to stay idempotent
...
BEGIN
  ...
  VALUES (
    ...
    p_engagement_score,
    CASE
      WHEN p_status IN ('draft', 'published', 'scheduled', 'archived') THEN p_status
      ELSE 'draft'
    END,
...
CREATE OR REPLACE FUNCTION create_carousel_project(
  p_user_id TEXT,
  p_title TEXT,
  p_role TEXT,
  p_slides JSONB,
...
CREATE OR REPLACE FUNCTION save_voice_training_sample(
  p_user_id TEXT,
  p_samples JSONB,
  p_fingerprint JSONB,
  p_sample_text TEXT,
...
CREATE OR REPLACE FUNCTION clear_voice_training_samples(
  p_user_id TEXT
)
RETURNS VOID AS $$
BEGIN
  ...
  DELETE FROM voice_training_logs
