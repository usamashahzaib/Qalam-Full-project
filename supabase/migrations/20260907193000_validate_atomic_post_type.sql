-- Add database-level validation to the atomic post update transaction.
...
begin
  ...
  if current_post.status = 'publishing' then raise exception 'post_is_publishing'; end if;
  if p_patch ? 'type' and (
    nullif(trim(p_patch ->> 'type'), '') is null
    or length(p_patch ->> 'type') > 80
  ...
  set
    ...
    metadata = case
      when p_patch ? 'type' then jsonb_set(coalesce(metadata, '{}'::jsonb), '{type}', to_jsonb(trim(p_patch ->> 'type')), true)
      else metadata
    end,
    updated_at = now()
  where id = p_post_id and workspace_id = p_workspace_id
