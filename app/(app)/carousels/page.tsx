...
export default function CarouselsPage() {
  ...
  const deleteCarousel = async (id: string) => {
    ...
    const data = await res.json().catch(() => ({}))
    if (!res.ok) throw new Error(data.error || "Carousel could not be deleted.")
        ...
        <div className="mb-6 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]" data-carousel-generator>
          ...
          <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
            ...
            <div className="space-y-1.5">
              <p className="mb-2 t-eyebrow text-zinc-400">Deck structure</p>
            ...
            {carousels.map((carousel) => (
              <div key={carousel.id} className="group relative rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-teal/40 hover:shadow-md">
                <Link href={withClientParam(`/carousels/${carousel.id}`, activeClientId)} className="block">
                  ...
                  >
                    <p className="t-eyebrow" style={{ color: (carousel.tone && THEME_META[carousel.tone]?.accent) || "#5eead4" }}>{carousel.slide_count || 7} slides</p>
                ...
                <DeleteArtifactButton
                  itemType="carousel"
                  itemTitle={carousel.topic || `Created ${formatDate(carousel.created_at)}`}
                  onDelete={() => deleteCarousel(carousel.id)}
                  className="absolute right-3 top-3 min-h-10 rounded-xl bg-white/90 px-3 text-xs font-bold text-zinc-500 shadow-sm transition hover:bg-red-50 hover:text-red-600"
                />
              </div>
            ))}
          </div>
