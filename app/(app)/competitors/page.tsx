...
export default function CompetitorsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1100px] px-4 py-8 lg:px-6">
      ...
      {/* Header */}
      <div className="mb-8">
        <p className="t-eyebrow text-zinc-400">Competitor Research</p>
        ...
        <aside className="space-y-4">
                ...
                {history.map((item) => (
                  ...
                  >
                    ...
                    <div className="mt-1.5 flex items-center gap-2">
                      <span className="t-eyebrow font-semibold text-teal">{item.hook_structure?.pattern || "Analyzed"}</span>
                      <span className="t-eyebrow text-zinc-400">{formatDate(item.created_at)}</span>
                    </div>
                  </button>
                ))}
              </div>
          ...
          {canUse && (
            <div className="rounded-2xl border border-zinc-200 bg-white p-4">
              <p className="t-eyebrow text-zinc-400">Monthly usage</p>
          ...
          <ol className="space-y-2">
            {analysis.improvements.map((tip, i) => (
              <li key={i} className="flex items-start gap-3 text-sm text-zinc-700">
                <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-zinc-900 t-eyebrow text-white">
                  {i + 1}
                </span>
                {tip}
              </li>
