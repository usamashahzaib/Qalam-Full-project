      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-5 grid gap-5">
          ...
          <section className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-6">
            <div className="flex items-start justify-between gap-3"><div><p className="t-eyebrow text-zinc-400">For employers and recruiters</p><h2 className="mt-1 text-xl font-bold text-zinc-900">Verified recruiter search</h2><p className="mt-1 text-sm text-zinc-500">Reviewed by hand, not instant. Qalam checks the organization identity and intended use before search is enabled, and search stays locked until it is.</p></div>{organization && <span className={`rounded-full px-2.5 py-1 t-eyebrow ${organization.verification_status === "verified" ? "bg-teal/10 text-teal" : "bg-gold/10 text-gold-700"}`}>{organization.verification_status}</span>}</div>
            ...
            <div className="mt-4 max-h-[520px] space-y-3 overflow-y-auto">
              {candidates.map((candidate) => <article key={candidate.workspace_id} className="rounded-xl border border-zinc-200 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold text-zinc-900">{candidate.public_name || "Qalam candidate"}</h3><p className="mt-1 text-sm text-zinc-600">{candidate.professional_headline}</p></div>{candidate.linkedin_url && <a href={candidate.linkedin_url} target="_blank" rel="noreferrer" className="text-xs font-bold text-teal">LinkedIn</a>}</div><p className="mt-3 text-xs text-zinc-500">{candidate.target_roles?.join(" · ")} {candidate.locations?.length ? `| ${candidate.locations.join(", ")}` : ""}</p><div className="mt-3 flex flex-wrap gap-1.5">{candidate.skills?.slice(0, 8).map((skill) => <span key={skill} className="rounded-full bg-zinc-100 px-2.5 py-1 t-eyebrow text-zinc-600">{skill}</span>)}</div></article>)}
