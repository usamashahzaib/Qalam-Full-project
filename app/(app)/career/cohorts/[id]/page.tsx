    ...
    <main className="min-h-full bg-zinc-50/70 px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        <header className="rounded-3xl bg-[#073f3b] px-7 py-8 text-white">
          <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-gold">Cohort progress</p><h1 className="mt-2 text-3xl font-bold">{data.cohort.name}</h1><p className="mt-2 text-sm text-white/70">{data.cohort.description}</p></div><span className="rounded-lg bg-white/10 px-3 py-2 font-mono text-sm font-bold">{data.cohort.code}</span></div>
        </header>
