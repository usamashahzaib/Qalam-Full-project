      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="rounded-2xl border border-zinc-200 bg-white p-6">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">{Object.keys(metrics).map((key) => <label key={key}><span className="mb-1 block text-[10px] font-bold uppercase text-zinc-500">{key}</span><input type="number" min={0} className={field} value={metrics[key as keyof typeof metrics]} onChange={(event) => setMetrics({ ...metrics, [key]: Number(event.target.value) })} /></label>)}</div>
...
function Result({ analysis }: { analysis: Analysis }) {
  return <div><div className="flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-teal">Content worth</p><p className="mt-1 text-sm text-zinc-600">{analysis.positioning}</p></div><p className="text-4xl font-bold text-gold">{analysis.content_worth_score || 0}</p></div><p className="mt-4 rounded-xl bg-zinc-50 p-3 text-sm text-zinc-600">{analysis.audience_signal}</p><div className="mt-4 grid grid-cols-2 gap-2">{Object.entries(analysis.scores || {}).map(([key, value]) => <div key={key} className="rounded-lg border border-zinc-200 p-3"><p className="text-[10px] font-bold uppercase text-zinc-400">{key}</p><p className="mt-1 font-bold text-zinc-900">{value}</p></div>)}</div><List title="What worked" items={analysis.what_worked} /><List title="Fix next" items={analysis.weaknesses} /><List title="Next post angles" items={analysis.next_post_angles} /></div>
}
