      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="rounded-2xl border border-zinc-200 bg-white p-6">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">{Object.keys(metrics).map((key) => <label key={key}><span className="mb-1 block text-[11px] font-bold uppercase text-zinc-500">{key}</span><input type="number" min={0} className={field} value={metrics[key as keyof typeof metrics]} onChange={(event) => setMetrics({ ...metrics, [key]: Number(event.target.value) })} /></label>)}</div>
