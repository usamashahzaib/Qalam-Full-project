    ...
    <main className="min-h-full bg-zinc-50/70 px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl">
        <header className="rounded-3xl bg-[#073f3b] px-7 py-8 text-white">
          <p className="t-eyebrow text-gold-700">Signal match</p>
              ...
              {open.map((suggestion) => (
                <article key={suggestion.id} className="flex flex-col rounded-2xl border border-zinc-200 bg-white p-5">
                  <div className="flex items-start justify-between gap-2">
                    <span className="rounded-full bg-teal/10 px-2.5 py-1 t-eyebrow text-teal">{suggestion.score}% fit</span>
                    {suggestion.connected && <span className="rounded-full bg-gold/15 px-2.5 py-1 t-eyebrow text-gold-700">Connected</span>}
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {suggestion.expertise.slice(0, 5).map((item) => (
                      <span key={item} className="rounded-full bg-zinc-100 px-2.5 py-1 t-eyebrow text-zinc-600">{item}</span>
                    ))}
                  </div>
