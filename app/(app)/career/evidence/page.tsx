      ...
      <div className="mx-auto max-w-7xl">
        <header className="border-b border-zinc-200 pb-7">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div><p className="t-eyebrow text-teal">Proof of employability</p><h1 className="mt-2 text-3xl font-bold tracking-tight text-zinc-950">One verified truth behind every career asset.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-600">Store proof once. Use it across resumes, LinkedIn, cover letters, interviews, and recruiter discovery without inventing claims.</p></div>
        ...
        <div className="grid gap-8 py-7 lg:grid-cols-[minmax(0,1fr)_340px]">
          <section><div className="mb-4 flex gap-2 overflow-x-auto">{[["all", "All"], ...types].map(([value, label]) => <button key={value} type="button" onClick={() => setFilter(value)} className={`min-h-10 whitespace-nowrap rounded-full px-4 text-xs font-bold transition ${filter === value ? "bg-zinc-950 text-white" : "border border-zinc-200 bg-white text-zinc-600 hover:border-teal/40"}`}>{label}</button>)}</div>{loading ? <div className="space-y-3">{[1, 2, 3].map((item) => <div key={item} className="h-28 animate-pulse rounded-xl bg-zinc-200/70" />)}</div> : visibleItems.length ? <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white">{visibleItems.map((item) => <article key={item.id} className="border-b border-zinc-100 px-5 py-5 last:border-b-0"><div className="flex items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><span className="t-eyebrow text-teal">{item.evidence_type.replace("_", " ")}</span><span className={`rounded-full px-2 py-0.5 t-eyebrow ${item.verification_status === "partner_verified" ? "bg-teal/10 text-teal" : item.verification_status === "documented" ? "bg-gold/10 text-gold-700" : "bg-zinc-100 text-zinc-500"}`}>{item.verification_status.replace("_", " ")}</span></div><h2 className="mt-2 text-base font-bold text-zinc-950">{item.title}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-600">{item.summary || "Add context and measurable proof to strengthen this evidence."}</p></div><button type="button" onClick={() => void remove(item.id)} className="min-h-10 px-2 text-xs font-semibold text-zinc-400 transition hover:text-red-600">Delete</button></div>{item.skills?.length ? <div className="mt-3 flex flex-wrap gap-1.5">{item.skills.map((skill) => <span key={skill} className="rounded-full bg-zinc-100 px-2.5 py-1 t-eyebrow font-medium text-zinc-600">{skill}</span>)}</div> : null}</article>)}</div> : <div className="py-20 text-center"><p className="text-lg font-bold text-zinc-900">No evidence in this view.</p><p className="mt-2 text-sm text-zinc-500">Start with one measurable achievement or credential.</p></div>}</section>

          <aside className="self-start rounded-2xl border border-zinc-200 bg-white p-5"><p className="t-eyebrow text-zinc-500">Privacy controls</p><h2 className="mt-2 text-lg font-bold text-zinc-950">Your data, your permission.</h2><p className="mt-2 text-sm leading-6 text-zinc-600">Qalam never turns private evidence into recruiter access or aggregate learning without the choice shown here.</p><div className="mt-5 divide-y divide-zinc-100">{[["recruiterDiscovery", "Recruiter discovery", "Allow verified recruiters to find your public profile."], ["outcomeLearning", "Outcome learning", "Use de-identified application outcomes to improve recommendations."], ["partnerReporting", "Partner reporting", "Include de-identified results in cohort reporting."]].map(([key, title, copy]) => <label key={key} className="flex cursor-pointer gap-3 py-4"><input type="checkbox" checked={consents[key as keyof Consents]} onChange={(event) => void updateConsent(key as keyof Consents, event.target.checked)} className="mt-1 h-4 w-4 accent-teal" /><span><strong className="block text-sm text-zinc-900">{title}</strong><span className="mt-1 block text-xs leading-5 text-zinc-500">{copy}</span></span></label>)}</div><p className="mt-4 text-xs leading-5 text-zinc-400">You can change these choices anytime. Visibility remains off until your public candidate profile is enabled.</p></aside>
        </div>
      </div>
    </main>
  )
