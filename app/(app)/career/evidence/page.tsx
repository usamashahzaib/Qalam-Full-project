    ...
    <main className="min-h-full bg-[oklch(0.975_0.006_165)] px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-8 py-7 lg:grid-cols-[minmax(0,1fr)_340px]">
          <section><div className="mb-4 flex gap-2 overflow-x-auto">{[["all", "All"], ...types].map(([value, label]) => <button key={value} type="button" onClick={() => setFilter(value)} className={`min-h-10 whitespace-nowrap rounded-full px-4 text-xs font-bold transition ${filter === value ? "bg-zinc-950 text-white" : "border border-zinc-200 bg-white text-zinc-600 hover:border-teal/40"}`}>{label}</button>)}</div>{loading ? <div className="space-y-3">{[1, 2, 3].map((item) => <div key={item} className="h-28 animate-pulse rounded-xl bg-zinc-200/70" />)}</div> : visibleItems.length ? <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white">{visibleItems.map((item) => <article key={item.id} className="border-b border-zinc-100 px-5 py-5 last:border-b-0"><div className="flex items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><span className="text-[11px] font-bold uppercase tracking-[0.12em] text-teal">{item.evidence_type.replace("_", " ")}</span><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${item.verification_status === "partner_verified" ? "bg-teal/10 text-teal" : item.verification_status === "documented" ? "bg-gold/10 text-gold-700" : "bg-zinc-100 text-zinc-500"}`}>{item.verification_status.replace("_", " ")}</span></div><h2 className="mt-2 text-base font-bold text-zinc-950">{item.title}</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-600">{item.summary || "Add context and measurable proof to strengthen this evidence."}</p></div><button type="button" onClick={() => void remove(item.id)} className="min-h-10 px-2 text-xs font-semibold text-zinc-400 transition hover:text-red-600">Delete</button></div>{item.skills?.length ? <div className="mt-3 flex flex-wrap gap-1.5">{item.skills.map((skill) => <span key={skill} className="rounded-full bg-zinc-100 px-2.5 py-1 text-[10px] font-medium text-zinc-600">{skill}</span>)}</div> : null}</article>)}</div> : <div className="py-20 text-center"><p className="text-lg font-bold text-zinc-900">No evidence in this view.</p><p className="mt-2 text-sm text-zinc-500">Start with one measurable achievement or credential.</p></div>}</section>
