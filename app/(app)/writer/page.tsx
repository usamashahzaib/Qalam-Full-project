...
export default function WriterPage() {
  ...
  const writerHref = withClientParam("/writer", activeClientId)
  ...
  const onSchedule = async () => {
    ...
    try {
      if (postType.toLowerCase().includes("carousel")) {
        ...
        if (data.projectId) setStatus(`Scheduled + Carousel ready -> ${withClientParam(`/carousels/${data.projectId}`, activeClientId)}`)
  ...
  const onPublish = async () => {
    ...
    try {
      ...
      if (postType.toLowerCase().includes("carousel")) {
        const data = await createCarousel(id).catch(() => ({}))
        if (data.projectId) setPublish((prev) => ({ ...prev, message: `Published + Carousel -> ${withClientParam(`/carousels/${data.projectId}`, activeClientId)}` }))
      }
    } catch (error) {
  ...
  const onLoadPost = (postId: string) => {
    ...
    setScheduleDate(post.scheduledTime?.split("T")[0] || post.date || "")
  ...
  const onSendForApproval = async () => {
    ...
    try {
      ...
      setEditingId(id)
      setStatus("Sent for approval ? - visible in Approvals queue")
    } catch (e) {
  ...
  return (
    <div className="mx-auto grid max-w-7xl grid-cols-1 gap-6 px-6 py-8 lg:grid-cols-[1fr_320px]">
      <section className="rounded-2xl border border-zinc-200 bg-white p-4 sm:p-6">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-zinc-500"><span>{wordCount} words</span><span>{characterCount} characters</span><span>{editingId ? `Editing ${editingId.slice(0, 8)}` : "New draft"}</span><span>{user?.linkedinMemberId ? "LinkedIn connected ?" : "LinkedIn required for publish"}</span></div>
      ...
      <aside className="rounded-2xl border border-zinc-200 bg-white p-4">
        ...
        {isLoadingPosts ? <div className="space-y-2">{[1, 2, 3].map((i) => <div key={i} className="h-12 rounded-lg bg-zinc-100 animate-pulse" />)}</div> : posts.length === 0 ? <div className="flex flex-col items-center py-8 text-center"><div className="mb-3 flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-teal/10 to-teal/5"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-teal" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg></div><p className="text-sm font-medium text-zinc-700">No posts yet</p><p className="mt-1 text-xs text-zinc-400">Start writing above to see your posts here.</p></div> : <div className="space-y-2">{posts.slice(0, 20).map((post) => <button key={post.id} onClick={() => onLoadPost(post.id)} className={`w-full rounded-lg border px-3 py-2 text-left hover:bg-zinc-50 transition-colors ${editingId === post.id ? "border-teal/40 bg-teal/5" : "border-zinc-200"}`}><p className="truncate text-sm font-medium text-zinc-900">{post.title}</p><div className="mt-0.5 flex items-center gap-2"><span className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${post.status === "published" ? "bg-emerald-50 text-emerald-700" : post.status === "scheduled" ? "bg-amber-50 text-amber-700" : post.status === "pending_approval" ? "bg-blue-50 text-blue-700" : "bg-zinc-100 text-zinc-600"}`}>{post.status.replace("_", " ")}</span><span className="text-[10px] text-zinc-400 truncate">{post.type}</span></div></button>)}</div>}
      </aside>
    </div>
  )
}
