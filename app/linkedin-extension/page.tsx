...
import { buildBreadcrumbSchema, buildFaqSchema, buildPageMetadata, absoluteUrl, resolvePublicHref } from "@/lib/seo"
    ...
    <main className="overflow-hidden bg-[#fbfdfc] pt-20 text-zinc-900">
      ...
      <section className="relative px-6 py-16 sm:py-24">
        ...
        <div className="relative mx-auto grid max-w-[1200px] items-center gap-12 lg:grid-cols-[0.9fr_1.1fr]">
          <div><p className="mb-4 t-eyebrow text-teal">Qalam for LinkedIn</p><h1 className="max-w-xl text-4xl font-bold tracking-[-0.04em] text-zinc-950 sm:text-6xl">Draft a better LinkedIn comment without leaving the post.</h1><p className="mt-6 max-w-xl text-lg leading-8 text-zinc-600">Click Qalam on a post you are reading, pick a writing angle, and get three concise comment drafts. Choose one to insert or copy. You always decide what gets published.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row"><a href="/downloads/qalam-linkedin-extension.zip" className="inline-flex min-h-12 items-center justify-center rounded-xl bg-teal px-5 text-sm font-bold text-white shadow-lg shadow-teal/20 transition-colors hover:bg-teal-600">Download extension package</a><Link href={resolvePublicHref("/extension/connect")} className="inline-flex min-h-12 items-center justify-center rounded-xl border border-teal/30 bg-white px-5 text-sm font-bold text-teal transition-colors hover:bg-teal/5">Connect Qalam</Link></div><p className="mt-4 text-xs leading-5 text-zinc-500">Desktop Chrome or Chromium only. This beta package is installed manually and is not a Chrome Web Store listing.</p></div>
          <ProductScene />
        </div>
      </section>
