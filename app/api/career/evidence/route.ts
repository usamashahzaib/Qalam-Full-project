...
export async function GET(request: NextRequest) {
  return withAuth(async (req) => {
    ...
    const { data, error } = await createServiceClient().from("career_evidence").select("*").eq("workspace_id", planCheck.workspaceId).order("updated_at", { ascending: false })
...
export async function POST(request: NextRequest) {
  return withAuth(async (req, user) => {
    ...
    if (entitlements.evidenceItems !== "unlimited") {
      const { count, error: countError } = await createServiceClient().from("career_evidence").select("id", { count: "exact", head: true }).eq("workspace_id", planCheck.workspaceId)
    ...
    const { data, error } = await createServiceClient().from("career_evidence").insert({
