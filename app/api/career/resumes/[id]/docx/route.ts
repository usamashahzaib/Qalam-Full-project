...
import { buildResumeDocx, resumeDocxFilename } from "@/lib/server/resume-docx-export"
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  return withAuth(async (req) => {
    ...
    const bytes = await buildResumeDocx(parsed.data, String(data.template_key || "clean"))
    return new NextResponse(Buffer.from(bytes), {
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        "Content-Disposition": `attachment; filename="${resumeDocxFilename(String(data.title || "ats-resume"))}"`,
