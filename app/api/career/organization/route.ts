...
export async function GET(request: NextRequest) {
  return withAuth(async (req) => {
    ...
    const { data, error } = await createScopedClient(planCheck.workspaceId).from("career_organizations").select("*").maybeSingle()
...
export async function POST(request: NextRequest) {
  return withAuth(async (req, user) => {
    ...
    const { data, error } = await supabase.from("career_organizations").upsert({ name: input.name, organization_type: input.organizationType, website: input.website || null, verification_status: "pending", verified_at: null, updated_at: new Date().toISOString() }, { onConflict: "workspace_id" }).select("*").single()
    ...
    // career_organization_members has no workspace_id column of its own
    // (scoped indirectly via organization_id, just created above) - use .raw.
    await supabase.from("career_organization_members").raw.upsert({ organization_id: org.id, user_id: user.id, role: "owner" }, { onConflict: "organization_id,user_id" })
