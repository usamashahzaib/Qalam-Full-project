...
export default async function ProductDetailPage({ params }: { params: Promise<Params> }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Products", item: absoluteUrl("/pricing") },
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {allProducts.map(([key, product]) => (
                  <Link
                    key={key}
                    href={`/product/${key}`}
                    className="group rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm transition-all hover:border-teal/40 hover:shadow"
                  >
                    <p className="text-sm font-bold text-zinc-900 group-hover:text-teal">{product.title}</p>
                    <p className="mt-1 text-xs leading-relaxed text-zinc-500">{product.summary}</p>
                  </Link>
                ))}
              </div>
            </div>
