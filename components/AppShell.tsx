...
function ReferralNavBadge() {
  ...
  return (
    <span className="shrink-0 rounded-full bg-teal/15 px-1.5 py-0.5 t-eyebrow text-teal-300">
      {count}
    </span>
  )
}
  ...
  return (
    ...
    >
                        ...
                        <div key={link.href} className={`rounded-xl py-2 pl-3 pr-3 text-sm font-medium ${active ? "border-l-2 border-teal-600 bg-teal-50/50 text-zinc-900" : "text-zinc-400"}`}>
                          ...
                          <button
                            ...
                            onClick={() => upgradeTarget
                              ...
                              : router.push("/contact")}
                            className="ml-7 mt-2 rounded-lg bg-teal px-2.5 py-1 t-eyebrow text-white shadow-sm transition-colors hover:bg-teal-600"
                          >
      ...
      <header className="qalam-app-header-mobile flex md:hidden flex-col border-b border-zinc-200 bg-white/90 backdrop-blur fixed top-0 inset-x-0 z-30" style={{ paddingTop: "env(safe-area-inset-top, 0px)" }}>
        ...
        {searchFocused && (
          <div className="absolute left-0 right-0 bg-white border-b border-zinc-200 px-4 py-3 shadow-lg z-40 fade-in" style={{ top: "calc(4rem + env(safe-area-inset-top, 0px))" }} ref={searchRef}>
            <div className="relative flex items-center">
