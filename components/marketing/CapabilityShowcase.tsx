    ...
    <div className="overflow-hidden rounded-[1.75rem] border border-zinc-200 bg-white shadow-[0_24px_60px_-32px_rgba(13,74,69,0.32)]">
      <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3 sm:px-5">
        <div className="flex items-center gap-2">
          ...
          <span className="t-eyebrow text-zinc-500">{title}</span>
        </div>
        <span className="rounded-full bg-teal-50 px-2.5 py-1 t-eyebrow text-teal">Qalam</span>
      </div>
      {children}
    </div>
  )
      ...
      <div className="grid gap-4 p-4 sm:grid-cols-[0.8fr_1.2fr] sm:p-6">
        ...
        <div className="space-y-3 rounded-2xl border border-zinc-100 p-4">
          {scores.map(([label, value, color]) => (
            <div key={label}>
              <div className="mb-1.5 flex justify-between t-eyebrow font-semibold text-zinc-600"><span>{label}</span><span>{value}</span></div>
    ...
    <WindowShell title="Targeted resume builder">
      <div className="grid min-h-[330px] sm:grid-cols-[0.38fr_0.62fr]">
        <div className="border-b border-zinc-100 bg-zinc-50 p-4 sm:border-b-0 sm:border-r">
          ...
          ].map(([label, value]) => <div key={label} className="border-b border-zinc-200 py-3 last:border-0"><p className="t-eyebrow text-zinc-400">{label}</p><p className="mt-1 text-xs font-semibold text-zinc-800">{value}</p></div>)}
        </div>
  ...
  return (
    <WindowShell title="Job description evidence match">
      <div className="p-4 sm:p-6">
        ...
        <div className="mt-4 divide-y divide-zinc-100">{rows.map(([requirement, status, style]) => <div key={requirement} className="flex items-center justify-between gap-3 py-3"><span className="text-xs font-medium text-zinc-700">{requirement}</span><span className={`shrink-0 rounded-full px-2.5 py-1 t-eyebrow ${style}`}>{status}</span></div>)}</div>
      </div>
    </WindowShell>
  )
}
  ...
  return (
    <WindowShell title="Voice-aware content studio">
      <div className="grid min-h-[330px] sm:grid-cols-[0.34fr_0.66fr]">
        ...
        <div className="p-5 sm:p-6"><div className="flex items-center justify-between"><span className="rounded-full bg-teal-50 px-2.5 py-1 t-eyebrow text-teal">Voice profile active</span><span className="t-eyebrow text-zinc-400">Draft 3</span></div><p className="mt-5 text-sm font-bold leading-6 text-zinc-900">The career advice I stopped giving after interviewing 47 product candidates.</p><p className="mt-3 text-xs leading-6 text-zinc-600">Most candidates do not have a skills problem. They have an evidence problem. Their resume lists what they know, but the interview never shows where they used it.</p><div className="mt-5 grid grid-cols-3 gap-2">{[["Hook", "84"], ["Clarity", "91"], ["Specificity", "79"]].map(([label, score]) => <div key={label} className="rounded-lg bg-zinc-50 p-2 text-center"><p className="text-sm font-bold text-teal">{score}</p><p className="t-eyebrow text-zinc-400">{label}</p></div>)}</div></div>
      </div>
    </WindowShell>
  )
}
  ...
  return (
    <WindowShell title="Career Visibility Hub">
      <div className="p-4 sm:p-6">
        ...
        <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-5">{steps.map((step, index) => <div key={step} className={`rounded-xl border p-3 ${index < 2 ? "border-teal-200 bg-teal-50" : "border-zinc-200 bg-white"}`}><span className="t-eyebrow text-zinc-400">0{index + 1}</span><p className="mt-2 t-eyebrow font-semibold text-zinc-700">{step}</p></div>)}</div>
      </div>
    </WindowShell>
  )
}
          ...
          <AnimatePresence mode="wait" initial={false}>
            ...
            >
              <div className="grid min-w-0 gap-7 xl:grid-cols-[0.78fr_1.22fr] xl:items-center">
                <div>
                  <div className="flex flex-wrap items-center gap-2"><span className="t-eyebrow text-teal">{active.eyebrow}</span><span className="rounded-full border border-gold/30 bg-gold-50 px-2.5 py-1 t-eyebrow text-gold-700">{active.availability}</span></div>
