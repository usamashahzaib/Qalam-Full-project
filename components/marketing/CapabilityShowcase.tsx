  ...
  return (
    <div className="overflow-hidden rounded-[1.75rem] border border-zinc-200 bg-white shadow-[0_24px_60px_-32px_rgba(13,74,69,0.32)]">
      <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3 sm:px-5">
        ...
        <span className="rounded-full bg-teal-50 px-2.5 py-1 text-[11px] font-bold text-teal">Qalam</span>
      </div>
      {children}
    </div>
  )
    ...
    <WindowShell title="Targeted resume builder">
      <div className="grid min-h-[330px] sm:grid-cols-[0.38fr_0.62fr]">
        <div className="border-b border-zinc-100 bg-zinc-50 p-4 sm:border-b-0 sm:border-r">
          <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-400">Verified evidence</p>
          {[
            ["Role", "Product Marketing Lead"],
            ["Scope", "4 markets"],
            ["Outcome", "Pipeline +31%"],
            ["Team", "7 collaborators"],
          ].map(([label, value]) => <div key={label} className="border-b border-zinc-200 py-3 last:border-0"><p className="text-[11px] text-zinc-400">{label}</p><p className="mt-1 text-xs font-semibold text-zinc-800">{value}</p></div>)}
        </div>
        <div className="p-5 sm:p-7">
          ...
          <p className="mt-5 text-[11px] font-bold uppercase tracking-wider text-zinc-400">Experience</p>
          ...
          <div className="mt-5 flex flex-wrap gap-2">{["Go-to-market", "Positioning", "Analytics", "Research"].map((skill) => <span key={skill} className="rounded-md bg-teal-50 px-2 py-1 text-[11px] font-semibold text-teal">{skill}</span>)}</div>
        </div>
      </div>
    </WindowShell>
  )
  ...
  return (
    <WindowShell title="Job description evidence match">
      <div className="p-4 sm:p-6">
        ...
        <div className="mt-4 divide-y divide-zinc-100">{rows.map(([requirement, status, style]) => <div key={requirement} className="flex items-center justify-between gap-3 py-3"><span className="text-xs font-medium text-zinc-700">{requirement}</span><span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-bold ${style}`}>{status}</span></div>)}</div>
      </div>
    </WindowShell>
  )
}
  ...
  return (
    <WindowShell title="LinkedIn positioning review">
      <div className="p-4 sm:p-6">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">{[["Search relevance", "78"], ["Credibility", "83"], ["Conversion clarity", "64"]].map(([label, score]) => <div key={label} className="border-t-2 border-teal bg-zinc-50 p-3"><p className="text-2xl font-extrabold text-zinc-900">{score}</p><p className="mt-1 text-[11px] font-semibold text-zinc-500">{label}</p></div>)}</div>
        <div className="mt-4 rounded-xl border border-zinc-200 p-4"><p className="text-[11px] font-bold uppercase tracking-wider text-gold">Recommended headline direction</p><p className="mt-2 text-xs leading-5 text-zinc-700">Operations leader scaling multi-site teams through measurable process improvement and service reliability.</p></div>
      </div>
    </WindowShell>
  )
}
    ...
    <WindowShell title="Voice-aware content studio">
      <div className="grid min-h-[330px] sm:grid-cols-[0.34fr_0.66fr]">
        <div className="border-b border-zinc-100 bg-[#073f3b] p-4 text-white sm:border-b-0 sm:border-r">
          <p className="text-[11px] font-bold uppercase tracking-wider text-gold">Content system</p>
  ...
  return (
    <WindowShell title="Career Visibility Hub">
      <div className="p-4 sm:p-6">
        ...
        <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-5">{steps.map((step, index) => <div key={step} className={`rounded-xl border p-3 ${index < 2 ? "border-teal-200 bg-teal-50" : "border-zinc-200 bg-white"}`}><span className="text-[11px] font-bold text-zinc-400">0{index + 1}</span><p className="mt-2 text-[11px] font-semibold text-zinc-700">{step}</p></div>)}</div>
      </div>
    </WindowShell>
  )
}
          ...
          <AnimatePresence mode="wait" initial={false}>
            ...
            >
              <div className="grid min-w-0 gap-7 xl:grid-cols-[0.78fr_1.22fr] xl:items-center">
                <div>
                  <div className="flex flex-wrap items-center gap-2"><span className="text-[11px] font-bold uppercase tracking-[0.14em] text-teal">{active.eyebrow}</span><span className="rounded-full border border-gold/30 bg-gold-50 px-2.5 py-1 text-[11px] font-bold text-gold-700">{active.availability}</span></div>
