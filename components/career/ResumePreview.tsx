...
export function ResumePreview({ data, templateKey }: { data: ResumeData; templateKey: string }) {
  ...
  return (
    <article className="mx-auto min-h-[1123px] w-full max-w-[794px] bg-white px-10 py-9 font-sans text-[11px] leading-[1.45] text-zinc-800 shadow-sm print:min-h-0 print:max-w-none print:shadow-none" style={{ "--resume-accent": template.accent } as React.CSSProperties}>
...
function Heading({ title, color }: { title: string; color: string }) {
  return <h2 className="mb-2 border-b pb-1 text-[11px] font-bold uppercase tracking-[0.14em]" style={{ color, borderColor: `${color}55` }}>{title}</h2>
}
